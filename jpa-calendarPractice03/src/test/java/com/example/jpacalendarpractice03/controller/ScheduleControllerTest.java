...
class ScheduleControllerTest {
    ...
    void scheduleSelectTest(){
        ResponseSchedule response=new ResponseSchedule("8","study JPA","2023-01-10");
        List<ResponseSchedule> result=new ArrayList<>();
        result.add(response);

        //조건 : 무엇을 호출 하면 무엇을 돌려준다.
        given(scheduleController.scheduleList(session)).willReturn(result);
        List<ResponseSchedule> testResult=scheduleController.scheduleList(session);
